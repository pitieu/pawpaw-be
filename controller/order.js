...
exports.cancelOrderMerchant = async (req, res, next) => {
    ...
    const orderData = await getOrder({ _id: req.orderId, providerId: req.user.id});
    if (
        ...
        await Order.updateOne({_id: req.orderId, providerId: req.user.id}, updateData);
...
exports.cancelOrder = async (req, res, next) => {
    ...
    const orderData = await getOrder({ _id: req.orderId, customerId: req.user.id});
    if (
        orderData.status == "accepted" || 
        orderData.status == "ongoing" ||
        orderData.status == "completed" ||
        ...
        await Order.updateOne({_id: req.orderId, customerId: req.user.id}, updateData);
    }
    throw Error("Unknown status can not cancel order.")
}
